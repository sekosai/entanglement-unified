...
\begin{itemize}
  ...
  \item \textbf{Gravity (consistency, not derivation).} The
    mechanism produces a relation of the form $G = \gamma c^2 R/M$
    with predicted $\gamma = 4/5$, consistent with the measured
    $G$ to within cosmic parameter uncertainties. The cosmic inputs
    are not $G$-independent, so this is a Mach--Sciama explanation,
    not a free $G$ derivation.
...
\begin{itemize}
  ...
  \item \textbf{Quantitative, but circular inputs:} The cosmic
    relation $G = (4/5) c^2 R/M$. The prefactor $4/5$ is a genuine
    prediction, but the cosmic inputs are not $G$-independent, so
    this is a Mach--Sciama explanation rather than a free derivation
    of $G$.
