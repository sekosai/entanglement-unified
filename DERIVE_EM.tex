...
\title{The Bohr Magneton from Helical-Jerk Geometry:\\
A \texorpdfstring{$G$}{G}-Free Test of the Entanglement-Torque Mechanism}
\author{}
\date{}
...
we apply the \emph{same mechanism} at the electron scale, where every
...
trajectory at the Compton scale, and that the magnetic moment of this
trajectory is exactly the Bohr magneton:
\begin{equation}
...
it is a much cleaner check than the cosmological calculation. We then
discuss the parallel structure between gravity and electromagnetism
and the open question of why the per-pair couplings differ by 36
orders of magnitude.
\end{abstract}

\section{The Bohr Magneton from the Helical Trajectory}

The mechanism: a particle with angular momentum $s = \sqrt{3}/2 \cdot
\hbar$ undergoing orthogonal jerk traces a helical trajectory of radius
$\lambdace = \hbar/(m_e c)$ at angular frequency $\omega_e = m_e c^2/\hbar$.
We show that this geometry, applied to the electron, reproduces the
Bohr magneton exactly.

\subsection{Helix Parameters}

\begin{align}
  \omega_e &= \frac{m_e c^2}{\hbar}
            = 7.763 \times 10^{20}\; \mathrm{rad/s} \\
  \lambdace &= \frac{\hbar}{m_e c}
             = 2.426 \times 10^{-12}\; \mathrm{m}
\end{align}
The helix carries one unit of $\hbar$:
\begin{equation}
  L = m_e \lambdace c = \hbar.
\end{equation}

\subsection{Magnetic Moment from the Current Loop}

The helix is a current loop carrying $I = e\omega_e/(2\pi)$ enclosing
area $A = \pi \lambdace^2$. Its magnetic moment is:
\begin{equation}
  \mu = I \cdot A
...
\begin{equation}
  \mu = \frac{e\,(m_e c^2/\hbar)\,(\hbar/(m_e c))^2}{2}
      = \frac{e\hbar}{2 m_e}
      = \mu_B.
\end{equation}
This is the Bohr magneton, exact and dimensionless-coefficient-free.

\subsection{Why This Matters}

Inputs: $e$, $\hbar$, $m_e$, $c$. None involve $G$, and all are
measured independently to high precision. Output: $\mu_B$,
independently measured to $\sim 10^{-10}$ precision. The mechanism
therefore makes a clean numerical prediction that can be checked
without circularity. This is the strongest empirical leg of the
entanglement-torque program.
